...
type StorageCommon interface {
	...
	AddFunds(ctx context.Context, addr addr.Address, amount abi.TokenAmount) (cid.Cid, error)

	// ReserveFunds reserves the given amount of funds is ensures it is available for the deal
	ReserveFunds(ctx context.Context, wallet, addr addr.Address, amt abi.TokenAmount) (cid.Cid, error)

	// ReleaseFunds releases funds reserved with ReserveFunds
	ReleaseFunds(ctx context.Context, addr addr.Address, amt abi.TokenAmount) error

	// GetBalance returns locked/unlocked for a storage participant.  Used by both providers and clients.
	GetBalance(ctx context.Context, addr addr.Address, tok shared.TipSetToken) (Balance, error)

	// VerifySignature verifies a given set of data was signed properly by a given addr's private key
	VerifySignature(ctx context.Context, signature crypto.Signature, signer addr.Address, plaintext []byte, tok shared.TipSetToken) (bool, error)
	...
	// SignsBytes signs the given data with the given addr's private key
	SignBytes(ctx context.Context, signer addr.Address, b []byte) (*crypto.Signature, error)
	...
	OnDealSectorPreCommitted(ctx context.Context, provider addr.Address, dealID abi.DealID, proposal market.DealProposal, publishCid *cid.Cid, cb DealSectorPreCommittedCallback) error

	// OnDealSectorCommitted waits for a deal's sector to be sealed and proved, indicating the deal is active
	OnDealSectorCommitted(ctx context.Context, provider addr.Address, dealID abi.DealID, sectorNumber abi.SectorNumber, proposal market.DealProposal, publishCid *cid.Cid, cb DealSectorCommittedCallback) error
...
type StorageProviderNode interface {
	...
	// GetMinerWorkerAddress returns the worker addr associated with a miner
	GetMinerWorkerAddress(ctx context.Context, addr addr.Address, tok shared.TipSetToken) (addr.Address, error)

	// GetDataCap gets the current data cap for addr
	GetDataCap(ctx context.Context, addr addr.Address, tok shared.TipSetToken) (*verifreg.DataCap, error)

	// GetProofType gets the current seal proof type for the given miner.
	GetProofType(ctx context.Context, addr addr.Address, tok shared.TipSetToken) (abi.RegisteredSealProof, error)
}
...
type StorageClientNode interface {
	...
	SignProposal(ctx context.Context, signer addr.Address, proposal market.DealProposal) (*market.ClientDealProposal, error)

	// GetDefaultWalletAddress returns the addr for this client
	GetDefaultWalletAddress(ctx context.Context) (addr.Address, error)

	// GetMinerInfo returns info for a single miner with the given addr
	GetMinerInfo(ctx context.Context, maddr addr.Address, tok shared.TipSetToken) (*StorageProviderInfo, error)
}
